...
def d3():
	...
	previously_selected_area_name = request.args.get("previously_selected_area_name")
	
	if area_name is None:
		area_name = "Centrum-West"

	if previously_selected_area_name is None:
		previously_selected_area_name = "Centrum-West";
	...
	previously_selected_plot_data = data.stats_ams.loc[data.stats_ams['area_name'] == previously_selected_area_name]
	previously_selected_plot_data = previously_selected_plot_data.drop(['area_name', 'area_code'], axis=1)
	previously_selected_plot_data = previously_selected_plot_data.to_json(orient='records')
	#previously_selected_area_name

	meta_data = data.stats_ams_meta.to_json(orient='records')
	return render_template("d3.html", data=plot_data, meta_data=meta_data,
		x_variables=data.model_vars, area_names=data.area_names, selected_area_name=area_name, previous_data = previously_selected_plot_data)
